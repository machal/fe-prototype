...
export default function Home() {
  ...
  const getSpeakerById = (id: string): Speaker | undefined => {
    return frontKonData.speakers.find(s => s.id === id) as Speaker | undefined
  }

  const getTopicsByIds = (ids: string[]): Topic[] => {
    return frontKonData.topics.filter(t => ids.includes(t.id)) as Topic[]
  }
        ...
        {currentView === 'speakers' && (
          <div className="grid">
            {frontKonData.speakers.map(speaker => {
              ...
              return (
                <Link key={speaker.id} href={`/speakers/${speaker.id}/`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <SpeakerCard
                    speaker={speaker}
        ...
        {currentView === 'topics' && (
          <div className="grid">
            {frontKonData.topics.map(topic => {
              ...
              return (
                <Link key={topic.id} href={`/topics/${topic.id}/`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <TopicCard
                    topic={topic}
                    talkCount={talks.length}
                  />
        ...
        {currentView === 'years' && (
          <div className="grid">
            {frontKonData.years.map(year => {
              ...
              return (
                <Link key={year.id} href={`/years/${year.id}/`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <YearCard
                    year={year}
                    talkCount={talks.length}
                  />
